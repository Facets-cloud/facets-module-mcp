...
def download_and_extract_module_zip(module_id: str, extract_to: str) -> str:
    """
    ...
    try:
        ...
        full_extract_path = ensure_path_in_working_directory(extract_to, working_directory)
        ...
        with requests.get(download_url, auth=(username, token), stream=True) as response:
            if response.status_code != 200:
                return f"Failed to download module zip: {response.status_code} {response.reason}"
        ...
        return f"Module {module_id} downloaded and extracted to {full_extract_path}"

    except Exception as e:
        return f"Error downloading or extracting module: {str(e)}" 